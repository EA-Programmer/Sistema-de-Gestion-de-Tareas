...
const elements = {
    ...
    summaryHigh: document.querySelector("#summaryHigh"),
};
...
function buildQuery() {
    ...
    if (elements.filterSubject.value.trim())
        params.set("subject", elements.filterSubject.value.trim());
...
async function loadTasks() {
    try {
        const [tasks, summary] = await Promise.all([
            request(`/tasks${buildQuery()}`),
            request("/tasks/summary"),
        ]);
        renderTasks(tasks);
...
async function saveTask(event) {
    ...
    try {
        if (editingId) {
            await request(`/tasks/${editingId}`, { method: "PUT", body: JSON.stringify(payload) });
